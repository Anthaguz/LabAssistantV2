...
public sealed class DeployProofRunner
{
    ...
    public FindingRecorder Run()
    {
        ...
        try
        {
            ...
            // tagged sweep misses it, but this pre/post diff still catches it.
            preRunVms = SafeList(() => probe.ListVmNames(string.Empty));
    ...
    {
        var orphanVms = SafeList(() => probe.ListVmNames(tagger.GlobalPrefix + "-"));
        var orphanSwitches = SafeList(() => probe.ListSwitchNames(tagger.GlobalPrefix + "-"));
    ...
    {
        var currentVms = SafeList(() => probe.ListVmNames(string.Empty));
    ...
    private static List<string> SafeList(Func<IReadOnlyList<string>> get)
    {
