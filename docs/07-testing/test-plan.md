...
- **Related AC:** `AC-021`, `FR-108`, `FR-109`, `FR-110`, `FR-111`, `FR-112`, `FR-190`
- **Type:** Manual (real Windows machine) + automated coverage
...
- **Reconciliation note (frame-based navigation, task `nav-frame-based`):** this milestone record predates frame-based navigation.
  Templates is now an on-demand `TemplatesPage` created on route entry and torn down on leave, hosting the Library, Editor, and Builder subviews in a `TabView`; Library, Editor, and Builder all bind to transient `TemplatesLibraryViewModel`, `TemplatesEditorViewModel`, and `TemplatesBuilderViewModel` through `x:Bind` (resolved from DI, reaching siblings and dialogs through the injected `ITemplatesLibraryHost`/`ITemplatesEditorHost`/`ITemplatesBuilderHost` seams the page implements).
  The former `TemplatesCapabilityRuntime`, `TemplatesLibraryWorkspace*`, `TemplatesEditorWorkspace*`, `TemplatesBuilderWorkspace*`, `TemplatesWorkspaceShellBridge`, the interim page-owned Builder adapter, and long-lived-workspace/route-refresh model no longer exist; automated coverage now lives in `TemplatesLibraryViewModelTests`, `TemplatesEditorViewModelTests`, `TemplatesEditorLogicTests`, `TemplateEditorHandoffTests`, and the Builder suites (`TemplatesBuilderDraftValidationTests`, `TemplatesBuilderViewModelInteractionTests`, `TemplatesBuilderEntrySmokeTests`, `TemplatesBuilderDirectoryTopologyProjectionTests`).
  Cross-capability edit-template hand-off from Deploy is now the app-lifetime `ITemplateEditorHandoff` mailbox drained by the page on entry, and the Templates behavioral contracts still hold (Library stable/default surface at `templates.library`, Editor explicit workflow-state destination at `templates.editor`, document/header/VM-slot/save/validate behavior) and remain the current verification target.
