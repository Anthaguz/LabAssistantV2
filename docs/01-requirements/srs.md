...
- **FR-203:** The V2 Builder shall keep deterministic suggestions separate from persisted user intent and require explicit user confirmation before save.
  - **Acceptance details:** The in-memory Builder draft is the active source of visible user intent during editing; there is no per-section or per-field Save button. Field edits update the draft immediately, or through a short UI-safe debounce where appropriate for responsive editing. Invalid intermediate values remain visible and remain in draft state instead of being discarded. Validation records errors and warnings against the current draft, and blocking errors prevent final Save, export, and planning until resolved. Builder suggestions may prefill or propose derived values such as network/domain/assignment defaults, but save output is based on the user-confirmed draft. Builder exposes a persistent command bar with Apply Suggestions, Validate, Save, Save As, Back, Previous, and Next. The `VMs` nav row includes a small borderless right-aligned `+` button, and the VM overview includes Add VM. New VMs default to neutral incrementing values from the existing draft set (`vm-1` / `VM 1`, `vm-2` / `VM 2`, and so on); after creation, the Builder selects the new VM child and opens `Basics`. Selected VM detail uses an internal left mini-nav with Basics, Resources, Membership, Roles, Networking, and Credentials; Resources contains RAM, CPU, and base disk/VHDX fields; Networking owns NIC list/detail, and NICs do not become global left-nav children. The former right-side VM selector list is not part of the Builder contract. Save confirmation belongs in the Review step. Final Save persists validated template JSON and still requires Review confirmation. Builder output must remain compatible with Deploy From Template review and V2 planning contracts. On save, Builder hides backend topology-role details and maps ordered Active Directory Domain Controller role assignments to the current persisted/runtime fields, including deriving each domain's `firstDomainControllerVmId`. Directory topology is optional when all VMs are standalone. If domains are absent, DomainMember VMs and Active Directory Domain Controller role VMs are invalid; if domains are present, each saved domain must have at least one VM assigned the Active Directory Domain Controller role. PDC emulator/FSMO selection or transfer is out of scope.
  - **Priority:** P1
