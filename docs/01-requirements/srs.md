...
- **FR-202:** The V2 Builder first authoring slice shall use a structured step-based workflow and expose only the approved first-slice V2 fields.
  - **Acceptance details:** The Builder workflow is ordered as General, Networks, Forests & Domains, Credentials, VMs, and Review, with a left active workflow tree and one active top-level step at a time rather than an all-sections scroll. The active top-level row uses existing shell resources, including `ShellAccentBrush`, selected background, and selected border treatment; completion/error badges are deferred to a later Review/validation UX slice. The `VMs` row expands to VM child items by VM name; each VM child expands to nested category rows in this order: Basics, Resources, Membership, Roles, Networking, Credentials. Clicking `VMs` opens a compact VM overview, clicking a VM child opens that VM detail on `Basics`, and clicking a VM category opens that category under `VMs > VM name > category`. The left workflow tree marks top-level rows active for non-VM steps, marks `VMs` active for the VM overview, marks the VM name active while any category for that VM is selected, and marks the selected VM category active under that VM node. Direct workflow-tree clicks remain supported. Previous/Next controls in the Builder footer compute the next route as General -> Networks -> Forests & Domains -> Credentials -> VMs overview -> every VM category in draft order (Basics, Resources, Membership, Roles, Networking, Credentials) -> Review. Previous is disabled on General, Next is disabled on Review, and Previous/Next do not block on validation errors before Review. When the draft has zero VMs, Next from VMs overview goes to Review, and Review shows a blocker that at least one VM is required before Save. Navigation between top-level steps, VM children, and VM detail categories must not discard Builder draft edits, and narrow VM/category navigation should refresh only the affected Builder view state instead of requiring broad full-Builder rerendering. General contains template name, description, deployment profile, and read-only schema/version metadata if shown; deployment profile is a visible horizontal Conservative/Balanced/Aggressive selector placed below description with a Segoe MDL2 information tooltip explaining the speed versus host-pressure tradeoff, while the persisted field value remains unchanged. Resource-heavy steps use list plus selected-detail layouts where applicable. First-slice authoring fields are lab networks, reusable credential slot references, forests/domains, VM membership mode, VM domain assignment, the Active Directory Domain Controller VM role, and per-VM NIC/IP/DNS/gateway authoring. Future role-specific configuration belongs under `VMs > VM name > Roles > Role name`. Root CA, SQL, Web, and Operations roles are future extension points and are not authorable now; this slice must not add empty Root CA, SQL, Web, or Operations configuration UI. Domain/forest editor redesign, credential semantic redesign, Review validation redesign, runtime/schema/trust/WPF work, field badges, inline markers, section badges, and completion/error badges are out of this contract slice; existing trust declarations must be preserved as deferred/read-only intent when an existing V2 template is opened and saved.
  - **Priority:** P1
