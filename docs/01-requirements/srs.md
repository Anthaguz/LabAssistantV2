...
- **FR-202:** The V2 Builder first authoring slice shall use a structured step-based workflow and expose only the approved first-slice V2 fields.
  - **Acceptance details:** The Builder workflow is ordered as General, Networks, Forests & Domains, Credentials, VMs, and Review, with a left stepper and one active step at a time rather than an all-sections scroll. General contains template name, description, deployment profile, and read-only schema/version metadata if shown; deployment profile remains the Conservative/Balanced/Aggressive field. Resource-heavy steps use list plus selected-detail layouts. First-slice authoring fields are lab networks, reusable credential slot references, forests/domains, VM membership mode, VM domain assignment, the Active Directory Domain Controller VM role, and per-VM NIC/IP/DNS/gateway authoring. Root CA, SQL, Web, and Operations roles are future extension points and are not authorable now. Trust authoring is out of this first Builder slice even though trust runtime support exists; existing trust declarations must be preserved as deferred/read-only intent when an existing V2 template is opened and saved.
  - **Priority:** P1
