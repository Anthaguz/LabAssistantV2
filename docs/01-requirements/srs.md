...
- **FR-132:** WinUI `Assets` extraction shall target capability-local workspace composition from the start, so `MainWindow` does not become the long-term composition hub for Assets local navigation, overview summaries, Base Disks, and Switches coordination.
  ...
  - **Priority:** P1
  - **Reconciliation note (frame-based navigation, task `nav-frame-based`):** FR-132 through FR-145 were written against the earlier model where every capability lived inside `MainWindow` as a long-lived workspace whose shared composition owner was a per-capability `AssetsCapabilityRuntime` with per-subview workspace compositions.
    The shell has since moved to frame-based capability navigation: Assets is an on-demand `AssetsPage` created on route entry and torn down on leave, and its Overview, Base Disks, and Switches subviews bind directly to `AssetsOverviewViewModel`, `AssetsBaseDisksViewModel`, and `AssetsSwitchesViewModel` through `x:Bind`.
    Where the requirements below name `AssetsCapabilityRuntime`, a per-subview "workspace composition", a "long-lived Assets workspace", or "rather than per-navigation recreation", read them as satisfied by the capability page and its navigation lifecycle.
    The Assets capability page is the Assets-local owner referenced here; `MainWindow` stays the shell composition root (FR-113); views never depend on `MainWindow`; and the behavioral route contracts below are preserved: Overview remains the route-entry summary/navigation surface (its summary still reflects Base Disks and Switches load state), Base Disks keeps its validation/remove guardrails, and Switches keeps its delete guardrails.
...
- **FR-146:** WinUI `Templates` shared composition cleanup shall converge shared `Templates Library` and `Templates Editor` composition into a Templates-local composition owner instead of leaving shared capability composition responsibilities in `MainWindow`.
  ...
  - **Reconciliation note (frame-based navigation, task `nav-frame-based`):** FR-146 through FR-154 were written against the earlier model where Templates lived inside `MainWindow` as a long-lived workspace whose shared composition owner was `TemplatesCapabilityRuntime` (with its per-subview `TemplatesLibraryWorkspaceComposition`, `TemplatesEditorWorkspaceComposition`, and `TemplatesWorkspaceShellBridge`).
    ...
    Where the requirements below name `TemplatesCapabilityRuntime`, a per-subview `WorkspaceComposition`/`WorkspaceController`/`WorkspaceShellBridge`, `MainWindow` ownership, a "long-lived Templates workspace", or "rather than per-navigation recreation", read them as satisfied by the capability page, the injected subview host seams, the handoff mailbox, and the frame navigation lifecycle.
    The Templates capability page is the Templates-local owner referenced here; `MainWindow` stays the shell composition root (FR-113); views never depend on `MainWindow`; and the behavioral route contracts below are preserved: Library remains the stable/default Templates surface at `templates.library`, and Editor remains a workflow-state destination at `templates.editor` entered from explicit actions rather than a peer tab.
...
- **FR-167:** WinUI `Diagnostics` shared composition cleanup shall converge shared `Diagnostics Overview` and `Diagnostics Logs` composition into a Diagnostics-local composition owner instead of leaving shared capability composition responsibilities in `MainWindow`.
  ...
  - **Priority:** P1
  - **Reconciliation note (frame-based navigation, task `nav-frame-based`):** FR-167 through FR-175 were written against the earlier model where every capability lived inside `MainWindow` as a long-lived workspace whose shared composition owner was a per-capability `...CapabilityRuntime`.
    ...
    Where the requirements below name `DiagnosticsCapabilityRuntime`, a "long-lived Diagnostics workspace", or "rather than per-navigation recreation", read them as satisfied by the capability page and its navigation lifecycle.
    The Diagnostics capability page is the Diagnostics-local owner referenced here; `MainWindow` stays the shell composition root (FR-113); views never depend on `MainWindow`; and the behavioral route contracts below are preserved: Overview remains the route-entry and summary surface, Logs remains a child troubleshooting surface, and the Overview logs summary still reflects Logs load state.
...
- **FR-170:** WinUI `Diagnostics Overview` shall be owned by the on-demand Diagnostics capability page rather than the shell (`MainWindow`), with Overview-specific state owned by `DiagnosticsOverviewViewModel` bound through `x:Bind`. (See the FR-167 reconciliation note; supersedes the earlier `DiagnosticsCapabilityRuntime` framing.)
...
- **FR-173:** WinUI `Diagnostics Logs` shall be owned by the on-demand Diagnostics capability page rather than the shell (`MainWindow`), with Logs-specific state owned by `DiagnosticsLogsViewModel` bound through `x:Bind`. (See the FR-167 reconciliation note; supersedes the earlier `DiagnosticsCapabilityRuntime` framing.)
...
- **FR-194:** The system shall support first-class multi-NIC network intent in `V2` templates, including explicit per-NIC switch attachment and explicit per-NIC guest IP, gateway, and DNS authoring.
  - **Acceptance details:** Router-style VMs with multiple attached switches/NICs are in scope for the V2 schema and planner baseline. V2 lab networks may declare portable switch intent with `switchName` plus optional `switchType` (`External`, `Internal`, or `Private`). Deploy-time switch reconciliation may create missing switches only when both switch name and type are declared; legacy `switchName`-only templates remain valid but cannot auto-create missing switches from that incomplete intent. Existing switches are reused only when name and type match, same-name/different-type switches block deployment with actionable guidance, missing `Internal` or `Private` switches may be created before VM creation, and missing `External` switches require Deploy Review adapter mapping for the current host.
  - **Priority:** P1
...
- **FR-198:** The system shall provide a `Review and Resolve` planning surface for `V2` deployments before runtime execution begins.
  - **Acceptance details:** The review surface shall show the computed orchestration graph/waves, unresolved credentials/bootstrap assumptions, network switch reconciliation blockers, dependency blockers, and selected deployment profile. Host-specific External switch adapter mappings are resolved during Deploy Review for the current deployment run and are not persisted into shared templates.
  - **Priority:** P1
...
- **FR-201:** WinUI `Templates` shall define a distinct V2 Builder workflow for creating and editing V2 templates under the Templates capability without replacing the current Templates Editor.
  - **Acceptance details:** The Builder is a Templates-local workflow-state destination, expected to use `templates.builder` or an equivalent Templates-local route key. The current `templates.editor` remains the V1/simple/legacy editing surface for existing schema editing and must not absorb V2 topology orchestration. V1/simple/legacy templates must not be routed into V2-only Builder authoring requirements.
  - **Priority:** P1

- **FR-202:** The V2 Builder first authoring slice shall use a structured step-based workflow and expose only the approved first-slice V2 fields.
  - **Acceptance details:** The Builder workflow is ordered as General, Networks, Forests & Domains, Credentials, VMs, and Review, with a left workflow navigator and one active top-level step at a time rather than an all-sections scroll. The root left panel remains first-level workflow navigation only. Selecting `VMs` drills or swipes the left navigator into a VM list panel, and selecting a VM drills or swipes the navigator into that VM's section list ordered as Basics, Resources, Membership, Roles, Networking, and Credentials. Clicking `VMs` opens a compact VM overview, clicking a VM opens that VM detail on `Basics`, and clicking a VM section opens that section under `VMs > VM name > section`. Selecting `Networking` for a VM drills or swipes the left navigator into that VM's NIC list panel; that panel includes Add NIC and one row per NIC, and selecting a NIC from the left panel shows only that NIC's configuration in the main content area. Navigator drill-in panels use a compact header with a chevron/back icon for one-level-up movement instead of a global breadcrumb bar; the visible control may be only a chevron, but its accessibility label and tooltip must name the exact target such as `Back to VMs`, `Back to VM sections`, or `Back to <VM name>`. Navigator chevron/back movement must never leave the Builder, discard draft progress, cancel deployment work, or route back to the Templates library. Leaving the Builder or returning to the Templates library remains a separate explicit command and must preserve existing unsaved-change protections. The active top-level row uses existing shell resources, including `ShellAccentBrush`, selected background, and selected border treatment; completion/error badges are deferred to a later Review/validation UX slice. Previous/Next controls in the Builder footer compute the next route as General -> Networks -> Forests & Domains -> Credentials -> VMs overview -> each VM's Basics -> Resources -> Membership -> Roles -> Networking overview -> NIC details in draft order -> Credentials -> Review. They move through this linear Builder route sequence rather than the current navigator hierarchy and should expose the destination where practical. Previous is disabled on General, Next is disabled on Review, and Previous/Next do not block on validation errors before Review. When the draft has zero VMs, Next from VMs overview goes to Review, and Review shows a blocker that at least one VM is required before Save. Navigation between top-level steps, VM list, VM detail sections, and NIC details must not discard Builder draft edits, and narrow VM/section/NIC navigation should refresh only the affected Builder view state instead of requiring broad full-Builder rerendering. Scrollable Builder content must reserve layout for scrollbars and must not overlap footer actions, action buttons, or command surfaces. General contains template name, description, deployment profile, and read-only schema/version metadata if shown; deployment profile is a visible horizontal Conservative/Balanced/Aggressive selector placed below description with a Segoe MDL2 information tooltip explaining the speed versus host-pressure tradeoff, while the persisted field value remains unchanged. Networks authors lab networks with the display label/name as the primary user-facing field; the stable `networkId` remains preserved for NIC references but is hidden or exposed only as an advanced/read-only identifier. Networks use a host-switch picker when switch inventory is available, include a `Create new switch` path, populate switch name/type from selected existing switches with type read-only, allow switch name plus `External`/`Internal`/`Private` type selection only for create-new intent, keep notes documentation-only, and preserve existing `networkId`, `name`, `switchName`, and optional `switchType` data when templates are loaded and saved. NICs primarily reference Networks, while per-NIC switch override remains advanced/reference-only and does not by itself create missing switches. Forests & Domains uses a constrained topology canvas with forest containers, highlighted root domain nodes, child-domain branches, tree-domain roots, selectable node details, and stable edge-ready node identifiers for future/read-only trust edges. First-slice authoring fields are lab networks, reusable credential slot references, forests/domains, VM membership mode, VM domain assignment, the Active Directory Domain Controller VM role, and per-VM NIC/IP/DNS/gateway authoring. Future role-specific configuration belongs under `VMs > VM name > Roles > Role name`. Root CA, SQL, Web, and Operations roles are future extension points and are not authorable now; this slice must not add empty Root CA, SQL, Web, or Operations configuration UI. Trust authoring, credential semantic redesign, Review validation redesign, WPF work, field badges, inline markers, section badges, and completion/error badges are out of this contract slice; existing trust declarations must be preserved as deferred/read-only intent when an existing V2 template is opened and saved.
  - **Priority:** P1
...
- **FR-203:** The V2 Builder shall keep deterministic suggestions separate from persisted user intent and require explicit user confirmation before save.
  - **Acceptance details:** The in-memory Builder draft is the active source of visible user intent during editing; there is no per-section or per-field Save button. Field edits update the draft immediately, or through a short UI-safe debounce where appropriate for responsive editing. Invalid intermediate values remain visible and remain in draft state instead of being discarded. Blocking errors prevent final Save, export, and planning until resolved, but do not block Previous/Next before Review. Builder suggestions may prefill or propose derived values such as network/domain/assignment defaults, but save output is based on the user-confirmed draft. The Builder exit/library-return command may remain a left secondary footer action when present, but it is separate from navigator chevron/back movement and must preserve unsaved-change protections. During authoring steps, the footer right side exposes Previous as a secondary action and Next as the primary action. On Review, the footer right side exposes Previous as a secondary action, Save As as a secondary action, and Save as the primary action. Apply Suggestions and manual Validate are not footer actions in this contract. Save and Save As are Review-only actions, and Save remains blocked unless the current draft builds into valid template JSON. Review plus Save is the confirmation; there is no separate Review confirmation checkbox. The VM list navigator panel includes a compact Add VM affordance, and the VM overview includes Add VM. New VMs default to neutral incrementing values from the existing draft set (`vm-1` / `VM 1`, `vm-2` / `VM 2`, and so on); after creation, the Builder selects the new VM and opens `Basics`. VM detail sections live in the selected VM's navigator panel: Basics, Resources, Membership, Roles, Networking, and Credentials. Resources contains RAM, CPU, and base disk/VHDX fields; Networking owns NIC overview/list and selected NIC detail, with footer Previous/Next visiting NIC details in deterministic draft order. The former right-side VM selector list is not part of the Builder contract. Final Save persists validated template JSON. Builder output must remain compatible with Deploy From Template review and V2 planning contracts. On save, Builder hides backend topology-role details and maps ordered Active Directory Domain Controller role assignments to the current persisted/runtime fields, including deriving each domain's `firstDomainControllerVmId`. Directory topology is optional when all VMs are standalone. If domains are absent, DomainMember VMs and Active Directory Domain Controller role VMs are invalid; if domains are present, each saved domain must have at least one VM assigned the Active Directory Domain Controller role. PDC emulator/FSMO selection or transfer is out of scope.
  - **Priority:** P1

- **FR-204:** The V2 Builder implementation target shall preserve Templates and shell ownership boundaries.
  - **Acceptance details:** Builder state, workflow orchestration, validation sequencing, composition, and view coordination belong to the Builder-local `TemplatesBuilderViewModel` (`x:Bind` MVVM) and its declarative `TemplatesBuilderView`, hosted by the transient `TemplatesPage` through the injected `ITemplatesBuilderHost` seam. `MainWindow`, shared Templates composition, and the current Templates Editor must not become the Builder workflow owner.
  - **Priority:** P1

- **FR-205:** The V2 Builder shall provide Builder-local scoped automatic validation for draft edits before final template build validation.
  - **Acceptance details:** Builder-local validation is a distinct authoring seam from final document mapper/build validation. Draft changes trigger validation automatically for affected scopes where practical, including domain names and dependent references, IP format/subnet/duplicate checks, VM identity/name checks, and VM membership domain assignment plus role compatibility. Review aggregates current blockers and warnings from the Builder-local validation state. Save and Save As remain Review-only actions and remain blocked by current validation blockers and by final build validation. Manual Validate remains absent from the Builder footer, and field badges, inline markers, section badges, and completion/error badges are out of scope for this validation slice.
  - **Priority:** P1
