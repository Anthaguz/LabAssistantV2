...
| TBD-043 | Exact import/edit UX for unresolved V2 credential slots after template sharing | `docs/01-requirements/acceptance-criteria.md`, `docs/04-data/v2-template-planning-contract.md` | Deferred | Revisit when import-time or V2 Builder credential-slot remapping becomes an approved authoring slice. |
| TBD-045 | Whether V2 Builder eventually replaces the current Templates Editor for all template types or remains V2-specific | `docs/00-overview/v2-health-trust-builder-issue-briefs.md`, `docs/00-overview/v2-unified-orchestration-roadmap.md` | Deferred | Revisit when the first V2 Builder implementation is complete enough to compare against the current editor. |
| TBD-046 | Which GitHub milestone should own V2 Builder work if it does not fit the existing backend V2 epics | `docs/00-overview/v2-health-trust-builder-issue-briefs.md` | Next contract | Resolve before creating the first V2 Builder GitHub implementation issue. |
| TBD-047 | Whether future trust slices need schema fields beyond trust id, source/target domains, type, and direction | `docs/04-data/v2-template-planning-contract.md` | Deferred | Revisit only after the first bidirectional managed-forest trust runtime slice is complete. |
