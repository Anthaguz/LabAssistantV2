...
public sealed class V2RuntimeCapabilityService : IV2RuntimeCapabilityService
{
    ...
    {
        // The teardown decision lives in one shared predicate (finding 88) so the terminal cleanup gate here and
        // the forest-trust stage's teardown prediction cannot drift. See VmCancelTeardownPolicy for the full rule
        // and the drift risk it removes; the equivalence matrix test pins both call paths to the same truth table.
        return VmCancelTeardownPolicy.ShouldTearDown(state.Context, multiContext);
    }
