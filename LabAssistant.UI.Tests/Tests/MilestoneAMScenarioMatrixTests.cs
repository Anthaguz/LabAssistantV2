...
public sealed class MilestoneAMScenarioMatrixTests
{
    ...
    public void MainWindow_PreservesShellBoundary_WhileHostingLongLivedAssetsWorkspace()
    {
        ...
        Assert.Contains("_assetsCapabilityRuntime = CreateAssetsCapabilityRuntime(", mainWindowSource);
        Assert.Contains("out var assetsBaseDisksWorkspaceComposition,", mainWindowSource);
        Assert.Contains("out var assetsSwitchesWorkspaceComposition);", mainWindowSource);
        Assert.Contains("_assetsBaseDisksWorkspaceComposition = assetsBaseDisksWorkspaceComposition;", mainWindowSource);
        Assert.Contains("_assetsSwitchesWorkspaceComposition = assetsSwitchesWorkspaceComposition;", mainWindowSource);
        Assert.Contains("private AssetsCapabilityRuntime CreateAssetsCapabilityRuntime(", mainWindowSource);
        Assert.Contains("assetsBaseDisksWorkspaceComposition = new AssetsBaseDisksWorkspaceComposition(", mainWindowSource);
        Assert.Contains("assetsSwitchesWorkspaceComposition = new AssetsSwitchesWorkspaceComposition(", mainWindowSource);
        Assert.Contains("var baseDisksCompositionHost = new AssetsBaseDisksCompositionHost(", mainWindowSource);
        Assert.Contains("var switchesCompositionHost = new AssetsSwitchesCompositionHost(", mainWindowSource);
        Assert.Contains("var capabilityHost = new AssetsCapabilityHost();", mainWindowSource);
        Assert.Contains("var capabilityShellBridge = new AssetsCapabilityShellBridge(", mainWindowSource);
    ...
    public void MainWindow_PreservesShellBoundary_WhileHostingLongLivedTemplatesWorkspace()
    {
        ...
        Assert.Contains("_templatesCapabilityRuntime = CreateTemplatesCapabilityRuntime();", mainWindowSource);
        Assert.Contains("private TemplatesCapabilityRuntime CreateTemplatesCapabilityRuntime()", mainWindowSource);
        Assert.Contains("private TemplatesWorkspaceShellBridge CreateTemplatesWorkspaceShellBridge()", mainWindowSource);
        Assert.Contains("Func<bool> isTemplatesCapabilityActive = () => IsTemplatesCapabilityActive;", mainWindowSource);
        Assert.Contains("Func<bool> isTemplatesLibraryActive = () => IsTemplatesLibraryActive;", mainWindowSource);
        Assert.Contains("Func<bool> isTemplatesEditorActive = () => IsTemplatesEditorActive;", mainWindowSource);
        Assert.Contains("_templatesCapabilityRuntime.ApplyShellState();", mainWindowSource);
        Assert.Contains("_templatesCapabilityRuntime.ApplyUiState();", mainWindowSource);
        Assert.DoesNotContain("private TemplatesWorkspaceUiState CreateTemplatesWorkspaceUiState()", mainWindowSource);
        Assert.DoesNotContain("private FrameworkElement TemplatesWorkspaceHost => TemplatesWorkspacePanel;", mainWindowSource);
        Assert.DoesNotContain("private IList<TemplateLibraryItem> TemplatesLibraryItems => _templatesCapabilityRuntime.LibraryItems;", mainWindowSource);
        Assert.Contains("private TemplatesEditorWorkspaceComposition CreateTemplatesEditorWorkspaceComposition(", mainWindowSource);
        Assert.Contains("private TemplatesEditorWorkspaceHost CreateTemplatesEditorWorkspaceHost(", mainWindowSource);
        Assert.Contains("forceRefresh => runtime?.LoadEditorReferenceDataAsync(forceRefresh)", mainWindowSource);
        Assert.Contains("(document, statusText) => runtime?.ShowEditorDocumentAsync(document, statusText) ?? Task.CompletedTask", mainWindowSource);
        Assert.Contains("Action navigateToLibrary = () => NavigateToRoute(ShellRouteKeys.TemplatesLibrary);", mainWindowSource);
    ...
    {
        var compositionSource = LoadTemplatesCapabilityRuntimeSource();
        ...
        Assert.Contains("internal sealed class TemplatesCapabilityRuntime", compositionSource);
        ...
        Assert.Contains("public bool IsLoading => _isLoading;", compositionSource);
        Assert.Contains("public async Task EnsureEditorReferenceDataAsync(bool forceRefresh)", compositionSource);
        Assert.Contains("_libraryComposition.ApplyUiState(_isLoading, _libraryComposition.SelectedItem is not null);", compositionSource);
        Assert.DoesNotContain("internal readonly record struct TemplatesWorkspaceUiState(", compositionSource);
        Assert.Contains("public void ApplyUiState()", compositionSource);
        ...
        Assert.Contains("private TemplatesLibraryWorkspaceComposition CreateTemplatesLibraryWorkspaceComposition(", mainWindowSource);
        Assert.Contains("private TemplatesLibraryWorkspaceHost CreateTemplatesLibraryWorkspaceHost(", mainWindowSource);
    ...
    public void TemplatesLibrary_RefinedArchitecture_PreservesLocalStateControllerAndCompositionSeams()
    {
        ...
        Assert.Contains("await _templatesCapabilityRuntime.EnsureLibraryAsync(forceRefresh: true);", mainWindowSource);
    ...
    public void TemplatesEditor_UsesRefinedEditorLocalArchitecture()
    {
        ...
        var templatesWorkspaceCompositionSource = LoadTemplatesCapabilityRuntimeSource();
        var mainWindowSource = LoadMainWindowSource();
        ...
        Assert.Contains("await _templatesCapabilityRuntime.EnsureEditorReferenceDataAsync(forceRefresh: true);", mainWindowSource);
        Assert.Contains("(document, statusText) => runtime?.ShowEditorDocumentAsync(document, statusText) ?? Task.CompletedTask", mainWindowSource);
        Assert.Contains("private TemplatesEditorWorkspaceHost CreateTemplatesEditorWorkspaceHost(", mainWindowSource);
        Assert.DoesNotContain("SaveTemplateButton.Click += SaveTemplateButton_Click;", mainWindowSource);
        Assert.DoesNotContain("AddTemplateVmButton.Click += AddTemplateVmButton_Click;", mainWindowSource);
    ...
    public void TemplatesExtraction_ClosureCoverage_ProtectsSharedLibraryEditorAndLifetimeBoundaries()
    {
        ...
        Assert.Contains("internal sealed class TemplatesCapabilityRuntime", templatesWorkspaceCompositionSource);
    ...
    public void MainWindow_PreservesShellBoundary_WhileHostingLongLivedDeployWorkspace()
    {
        ...
        Assert.Contains("Func<IReadOnlyList<TemplateLibraryItem>> getLibraryItems = () => _templatesCapabilityRuntime.LibraryItems.ToList();", mainWindowSource);
    ...
    public void DeployExtraction_PreservesLongLivedWorkspaceRefreshAndDefaultRouteAnchors()
    {
        ...
        Assert.Contains("_fromTemplateLane.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", compositionSource);
    ...
    public void DeployOverview_ProtectsRefinedOverviewLocalArchitecture()
    {
        ...
        Assert.DoesNotContain("RefreshDeploySharedUiState,", mainWindowSource);
        Assert.Contains("_deployCapabilityRuntime.ApplyShellState();", mainWindowSource);
        Assert.DoesNotContain("private void UpdateDeployOverviewUi()", mainWindowSource);
        Assert.DoesNotContain("DeployOverviewOpenQuickDeployButton.Click += (_, _) => NavigateToRoute(ShellRouteKeys.DeployOnTheFly);", mainWindowSource);
        Assert.DoesNotContain("DeployOverviewOpenFromTemplateButton.Click += (_, _) => NavigateToRoute(ShellRouteKeys.DeployFromTemplate);", mainWindowSource);
        ...
        Assert.Contains("private readonly IDeployQuickDeployLane _quickDeployLane;", compositionSource);
        Assert.Contains("private readonly DeployOverviewWorkspaceComposition _overviewWorkspaceComposition;", compositionSource);
        Assert.Contains("private readonly IDeployFromTemplateLane _fromTemplateLane;", compositionSource);
        ...
        Assert.Contains("_quickDeployLane.SharedUiStateChanged += OnQuickDeployLane_SharedUiStateChanged;", compositionSource);
    ...
    public void DeployFromTemplate_UsesLocalSelectionReviewStateSeam()
    {
        ...
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition = new DeployFromTemplateWorkspaceComposition(", mainWindowSource);
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition.ActiveTemplateDocument", mainWindowSource);
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition.SelectedTemplateLibraryItem", mainWindowSource);
        ...
        Assert.Contains("private readonly IDeployFromTemplateLane _fromTemplateLane;", deployWorkspaceCompositionSource);
        Assert.Contains("IDeployFromTemplateLane fromTemplateLane,", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateLane = fromTemplateLane;", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateLane.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", deployWorkspaceCompositionSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceComposition : IDeployFromTemplateWorkspaceControllerHost, IDeployFromTemplateLane", fromTemplateCompositionSource);
        ...
        Assert.Contains("UpdateUi();", fromTemplateCompositionSource);
        ...
        Assert.Contains("private async Task OpenTemplateEditorAsync()", fromTemplateCompositionSource);
        Assert.Contains("await _host.ShowTemplateEditorAsync(document, \"Template loaded.\");", fromTemplateCompositionSource);
        Assert.Contains("public Task EnsureTemplatesLoadedAsync(bool forceRefresh) => LoadTemplatesAsync(forceRefresh);", fromTemplateCompositionSource);
        Assert.Contains("public void RefreshUi() => UpdateUi();", fromTemplateCompositionSource);
        ...
        Assert.Contains("private async Task HandleTemplateSelectionChangedAsync()", fromTemplateCompositionSource);
        Assert.Contains("private async Task LoadTemplatesAsync(bool forceRefresh)", fromTemplateCompositionSource);
    ...
    public void DeployFromTemplate_UsesLocalDeployOrchestrationSeam()
    {
        ...
        Assert.Contains("Func<bool> isTemplatesLoading = () => _templatesCapabilityRuntime.IsLoading;", mainWindowSource);
        Assert.Contains("Func<IReadOnlyList<TemplateLibraryItem>> getLibraryItems = () => _templatesCapabilityRuntime.LibraryItems.ToList();", mainWindowSource);
        Assert.Contains("Func<bool, Task> ensureLibraryAsync = forceRefresh => _templatesCapabilityRuntime.EnsureLibraryAsync(forceRefresh);", mainWindowSource);
        Assert.Contains("Func<string, Task<TemplateEditorDocument>> loadTemplateForEditorAsync =", mainWindowSource);
        ...
        Assert.Contains("Func<TemplateEditorDocument, string, Task> showTemplateEditorAsync =", mainWindowSource);
        Assert.DoesNotContain("RefreshDeploySharedUiState,", mainWindowSource);
        ...
        Assert.Contains("void IDeployFromTemplateWorkspaceControllerHost.ApplyWorkspaceState() => UpdateUi();", fromTemplateCompositionSource);
        Assert.Contains("_view.ReloadTemplatesRequested += async (_, _) => await EnsureTemplatesLoadedAsync(forceRefresh: true);", fromTemplateCompositionSource);
        Assert.Contains("_view.EvaluateReadinessRequested += async (_, _) => await EvaluateReadinessAsync(DeploymentPreflightMode.Quick);", fromTemplateCompositionSource);
        Assert.Contains("_view.ResolveSuggestionsRequested += async (_, _) => await ResolveSuggestionsAsync();", fromTemplateCompositionSource);
        Assert.Contains("_view.OpenTemplateEditorRequested += async (_, _) => await OpenTemplateEditorAsync();", fromTemplateCompositionSource);
        Assert.Contains("_view.StartDeployRequested += async (_, _) => await StartDeployAsync();", fromTemplateCompositionSource);
    ...
    public void DeployFromTemplate_UsesLocalCompositionHostCleanupSeam()
    {
        ...
        Assert.Contains("public bool IsLoadingTemplates => _isLoadingTemplates;", fromTemplateCompositionSource);
        Assert.Contains("public bool ShouldAutoOpenResultsPanel =>", fromTemplateCompositionSource);
        Assert.Contains("public string ResultsPanelTitle => \"From Template Progress / Results\";", fromTemplateCompositionSource);
        ...
        Assert.Contains("internal interface IDeployFromTemplateCompositionHost", fromTemplateCompositionHostSource);
        Assert.Contains("bool IsTemplatesLoading { get; }", fromTemplateCompositionHostSource);
        Assert.Contains("Task EnsureReferenceDataAsync(bool forceRefresh);", fromTemplateCompositionHostSource);
        Assert.Contains("Task EnsureTemplatesLibraryAsync(bool forceRefresh);", fromTemplateCompositionHostSource);
        Assert.Contains("Task<TemplateEditorDocument> LoadTemplateForEditorAsync(string filePath);", fromTemplateCompositionHostSource);
        Assert.Contains("void RefreshSharedUiState();", fromTemplateCompositionHostSource);
    ...
    public void DeployFromTemplate_ReducesBroadViewExposureAndUsesNarrowViewSeams()
    {
        ...
        Assert.Contains("_view.OpenTemplateEditorRequested += async (_, _) => await OpenTemplateEditorAsync();", fromTemplateCompositionSource);
        Assert.Contains("_view.StartDeployRequested += async (_, _) => await StartDeployAsync();", fromTemplateCompositionSource);
        Assert.Contains("_view.TemplateSelectionChanged += async (_, _) => await HandleTemplateSelectionChangedAsync();", fromTemplateCompositionSource);
        Assert.Contains("_view.OpenResultsPanelRequested += (_, _) => _host.OnOpenResultsPanelRequested();", fromTemplateCompositionSource);
    ...
    public void DeployOnTheFly_UsesLocalVmCollectionStateSeam()
    {
        ...
        Assert.Contains("_quickDeployLane.ApplyShellState(_shellBridge.IsDeployOnTheFlyActive);", deployWorkspaceCompositionSource);
    ...
    public void DeployOnTheFly_UsesLocalCompositionAndHostSeam()
    {
        ...
        Assert.Contains("private readonly IDeployQuickDeployLane _quickDeployLane;", deployWorkspaceCompositionSource);
        Assert.Contains("IDeployQuickDeployLane quickDeployLane,", deployWorkspaceCompositionSource);
        Assert.Contains("_quickDeployLane.SharedUiStateChanged += OnQuickDeployLane_SharedUiStateChanged;", deployWorkspaceCompositionSource);
        Assert.Contains("_quickDeployLane.ApplyShellState(_shellBridge.IsDeployOnTheFlyActive);", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("TryToggleRightPanelFromWorkflow", deployWorkspaceCompositionSource);
        ...
        Assert.Contains("private readonly IDeployQuickDeployLane _quickDeployLane;", deployResultsPanelCoordinatorSource);
        Assert.Contains("_quickDeployLane.ApplyResultsPanelState(_isDeployOnTheFlyActive(), showPanel, panelUnavailable);", deployResultsPanelCoordinatorSource);
