...
public sealed class MilestoneAMScenarioMatrixTests
{
    ...
    public void MainWindow_PreservesShellBoundary_WhileHostingLongLivedDeployWorkspace()
    {
        ...
        Assert.Contains("public const string DeployOverview = \"deploy.overview\";", shellViewModelSource);
        Assert.Contains("public const string DeployOnTheFly = \"deploy.on_the_fly\";", shellViewModelSource);
        Assert.Contains("public const string DeployFromTemplate = \"deploy.from_template\";", shellViewModelSource);
        Assert.Contains("new ShellSubview(ShellRouteKeys.DeployOverview, \"Overview\"", shellViewModelSource);
        Assert.Contains("new ShellSubview(ShellRouteKeys.DeployOnTheFly, \"Quick Deploy\"", shellViewModelSource);
        Assert.Contains("new ShellSubview(ShellRouteKeys.DeployFromTemplate, \"From Template\"", shellViewModelSource);
        Assert.Contains("private readonly DeployWorkspaceComposition _deployWorkspaceComposition;", mainWindowSource);
        Assert.Contains("private readonly DeployReferenceDataService _deployReferenceDataService;", mainWindowSource);
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceOwner _deployOnTheFlyWorkspaceOwner;", mainWindowSource);
        Assert.Contains("private readonly DeployResultsPanelCoordinator _deployResultsPanelCoordinator;", mainWindowSource);
        Assert.Contains("_deployWorkspaceComposition = new DeployWorkspaceComposition(", mainWindowSource);
        Assert.Contains("_deployOnTheFlyWorkspaceOwner = new DeployOnTheFlyWorkspaceOwner(", mainWindowSource);
        Assert.Contains("_deployResultsPanelCoordinator = new DeployResultsPanelCoordinator(", mainWindowSource);
        ...
        Assert.Contains("private bool IsDeployOverviewActive =>", mainWindowSource);
        ...
        Assert.Contains("private bool IsDeployCapabilityActive =>", mainWindowSource);
        Assert.Contains("RefreshDeploySharedUiState,", mainWindowSource);
        Assert.Contains("_deployWorkspaceComposition.ApplyShellState();", mainWindowSource);
        Assert.Contains("if (IsDeployFromTemplateActive)", mainWindowSource);
        Assert.Contains("_ = _deployFromTemplateWorkspaceOwner.EnsureTemplatesLoadedAsync(forceRefresh: false);", mainWindowSource);
        ...
        Assert.DoesNotContain("private readonly DeployOnTheFlyWorkspaceViewModel _deployOnTheFlyWorkspace = new();", mainWindowSource);
    ...
    [Fact]
    public void DeployExtraction_PreservesLongLivedWorkspaceRefreshAndDefaultRouteAnchors()
    {
        var mainWindowSource = LoadMainWindowSource();
        ...
        Assert.Contains("if (IsDeployFromTemplateActive)", mainWindowSource);
        Assert.Contains("_ = _deployFromTemplateWorkspaceOwner.EnsureTemplatesLoadedAsync(forceRefresh: false);", mainWindowSource);
        Assert.Contains("private void RefreshDeploySharedUiState()", mainWindowSource);
        Assert.Contains("private void ReconcileDeployFromTemplateSelection(IReadOnlyList<TemplateLibraryItem> items)", mainWindowSource);
        Assert.Contains("QuickDeployDraftCount: _deployOnTheFlyWorkspaceOwner.DraftCount,", mainWindowSource);
        ...
        Assert.DoesNotContain("private DeployWorkspaceComposition? _cachedDeployWorkspaceComposition;", mainWindowSource);
        Assert.DoesNotContain("_deployWorkspaceComposition = null;", mainWindowSource);
        Assert.DoesNotContain("DisposeDeployWorkspace", mainWindowSource);
        Assert.DoesNotContain("ResetDeployWorkspace", mainWindowSource);

        Assert.Contains("public void ApplyShellState()", compositionSource);
        Assert.Contains("if (_shellBridge.IsDeployOverviewActive)", compositionSource);
        Assert.Contains("_overviewWorkspaceComposition.ApplyShellState();", compositionSource);
        Assert.Contains("_onTheFlyWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployOnTheFlyActive);", compositionSource);
        Assert.Contains("_fromTemplateWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", compositionSource);
        Assert.Contains("if (!_shellBridge.IsDeployCapabilityActive)", compositionSource);
        Assert.Contains("var selectedTab = _shellBridge.IsDeployOverviewActive", compositionSource);
        ...
        Assert.DoesNotContain("ApplyResolveSuggestionsAsync", compositionSource);
        Assert.DoesNotContain("ReconcileFromTemplateSelection", compositionSource);
        ...
        Assert.DoesNotContain("new DeployOverviewWorkspaceComposition(", ExtractSection(
            compositionSource,
    ...
    public void DeployOverview_ProtectsRefinedOverviewLocalArchitecture()
    {
        var mainWindowSource = LoadMainWindowSource();
        var compositionSource = LoadDeployWorkspaceCompositionSource();
        var onTheFlyOwnerSource = LoadDeployOnTheFlyWorkspaceOwnerSource();
        ...
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceOwner _onTheFlyWorkspaceOwner;", compositionSource);
        ...
        Assert.Contains("_onTheFlyWorkspaceOwner.SharedUiStateChanged += OnTheFlyWorkspaceOwner_SharedUiStateChanged;", compositionSource);
        ...
        Assert.Contains("internal sealed class DeployOnTheFlyWorkspaceOwner : IDeployOnTheFlyWorkspaceControllerHost", onTheFlyOwnerSource);
        Assert.Contains("public event EventHandler? SharedUiStateChanged;", onTheFlyOwnerSource);
    ...
    public void DeployFromTemplate_UsesLocalSelectionReviewStateSeam()
    {
        ...
        var fromTemplateViewClassBlock = ExtractSection(
            fromTemplateViewCodeBehindSource,
            "public sealed partial class DeployFromTemplateView : UserControl",
            "private void OnTemplateSelectionChanged");

        Assert.Contains("private readonly DeployFromTemplateWorkspaceOwner _deployFromTemplateWorkspaceOwner;", mainWindowSource);
        Assert.Contains("_deployFromTemplateWorkspaceOwner = new DeployFromTemplateWorkspaceOwner(", mainWindowSource);
        Assert.DoesNotContain("_deployFromTemplateWorkspaceOwner.ActiveTemplateDocument", mainWindowSource);
        Assert.DoesNotContain("_deployFromTemplateWorkspaceOwner.SelectedTemplateLibraryItem", mainWindowSource);
        ...
        Assert.Contains("private readonly DeployFromTemplateWorkspaceOwner _fromTemplateWorkspaceOwner;", deployWorkspaceCompositionSource);
        Assert.Contains("DeployFromTemplateWorkspaceOwner fromTemplateWorkspaceOwner,", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateWorkspaceOwner = fromTemplateWorkspaceOwner;", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", deployWorkspaceCompositionSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceOwner : IDeployFromTemplateWorkspaceControllerHost", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateView _view;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateWorkspaceViewModel _workspace = new();", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateWorkspaceComposition _composition;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateWorkspaceController _controller;", fromTemplateOwnerSource);
        Assert.Contains("private readonly IDeployFromTemplateWorkspaceHost _host;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployReferenceDataService _referenceDataService;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployResolveSuggestionsService _resolveSuggestionsService;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployTemplateEditorLauncher _templateEditorLauncher;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateWorkspaceShellBridge _shellBridge;", fromTemplateOwnerSource);
        Assert.Contains("public Task EnsureTemplatesLoadedAsync(bool forceRefresh) => LoadTemplatesAsync(forceRefresh);", fromTemplateOwnerSource);
        Assert.Contains("public void RefreshUi() => UpdateUi();", fromTemplateOwnerSource);
        Assert.Contains("public void ReconcileSelection(IReadOnlyList<TemplateLibraryItem> items)", fromTemplateOwnerSource);
        Assert.Contains("private async Task OpenTemplateEditorAsync()", fromTemplateOwnerSource);
        Assert.Contains("await _templateEditorLauncher.ShowEditorAsync(document, \"Template loaded.\");", fromTemplateOwnerSource);
        Assert.Contains("private async Task HandleTemplateSelectionChangedAsync()", fromTemplateOwnerSource);
        Assert.Contains("private async Task LoadTemplatesAsync(bool forceRefresh)", fromTemplateOwnerSource);
        ...
        Assert.Contains("private readonly DeployFromTemplateWorkspaceViewModel _workspace;", fromTemplateCompositionSource);
        Assert.DoesNotContain("IDeployFromTemplateWorkspaceControllerHost", fromTemplateCompositionSource);
        Assert.DoesNotContain("private readonly IDeployFromTemplateCompositionHost _host;", fromTemplateCompositionSource);
        Assert.Contains("templateItemsSource,", fromTemplateCompositionSource);
        Assert.Contains("public void SetVisibility(bool isActive)", fromTemplateCompositionSource);
        Assert.Contains("public void UpdateUi(", fromTemplateCompositionSource);
        ...
        Assert.Contains("public event SelectionChangedEventHandler? TemplateSelectionChanged;", fromTemplateViewClassBlock);
        Assert.Contains("public TemplateLibraryItem? SelectedTemplateLibraryItem", fromTemplateViewClassBlock);
        Assert.Contains("public void SetTemplateItemsSource(object? itemsSource)", fromTemplateViewClassBlock);
        Assert.Contains("public void SetTemplateSelectorDisplayMemberPath(string displayMemberPath)", fromTemplateViewClassBlock);
    ...
    public void DeployFromTemplate_UsesLocalGroupedIssueRemediationStateSeam()
    {
        ...
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition.SetReadinessSummary(", mainWindowSource);
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition.ClearGroupedIssueState();", mainWindowSource);
        ...
        Assert.Contains("ApplyWorkspaceState()", fromTemplateCompositionSource);
        ...
        Assert.Contains("public void SetSharedIssueSummariesItemsSource(object? itemsSource)", fromTemplateViewCodeBehindSource);
        Assert.Contains("public void SetIssueRowsItemsSource(object? itemsSource)", fromTemplateRightPanelViewCodeBehindSource);
    }

    [Fact]
    public void DeployFromTemplate_UsesLocalDeployOrchestrationSeam()
    {
        var mainWindowSource = LoadMainWindowSource();
        var deployWorkspaceCompositionSource = LoadDeployWorkspaceCompositionSource();
        var fromTemplateOwnerSource = LoadDeployFromTemplateWorkspaceOwnerSource();
        ...
        var fromTemplateWorkspaceHostInterfaceSource = LoadDeployFromTemplateWorkspaceHostInterfaceSource();
        ...
        Assert.Contains("var deployResolveSuggestionsService = new DeployResolveSuggestionsService();", mainWindowSource);
        Assert.Contains("var deployTemplateEditorLauncher = new DeployTemplateEditorLauncher(_templatesWorkspaceComposition);", mainWindowSource);
        Assert.Contains("() => _isTemplatesLoading,", mainWindowSource);
        Assert.Contains("() => TemplatesLibraryItems.ToList(),", mainWindowSource);
        Assert.Contains("EnsureTemplatesLibraryAsync,", mainWindowSource);
        ...
        Assert.DoesNotContain("template => _deployWorkspaceComposition.ApplyResolveSuggestionsAsync(template),", mainWindowSource);
        Assert.Contains("RefreshDeploySharedUiState,", mainWindowSource);
        ...
        Assert.Contains("private readonly DeployFromTemplateWorkspaceOwner _fromTemplateWorkspaceOwner;", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("ApplyResolveSuggestionsAsync", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("ReconcileFromTemplateSelection", deployWorkspaceCompositionSource);
        ...
        Assert.Contains("private readonly DeployTemplateEditorLauncher _templateEditorLauncher;", fromTemplateOwnerSource);
        Assert.Contains("private readonly DeployFromTemplateWorkspaceShellBridge _shellBridge;", fromTemplateOwnerSource);
        Assert.Contains("Task<DeploymentOutcomeSummary> IDeployFromTemplateWorkspaceControllerHost.DeployAllAsync(MultiVmDeploymentContext context) => _host.DeployAllAsync(context);", fromTemplateOwnerSource);
        Assert.Contains("void IDeployFromTemplateWorkspaceControllerHost.ApplyWorkspaceState() => UpdateUi();", fromTemplateOwnerSource);
        Assert.Contains("_view.ReloadTemplatesRequested += ReloadTemplatesRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.EvaluateReadinessRequested += EvaluateReadinessRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.ResolveSuggestionsRequested += ResolveSuggestionsRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.OpenTemplateEditorRequested += OpenTemplateEditorRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.StartDeployRequested += StartDeployRequested;", fromTemplateOwnerSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceComposition", fromTemplateCompositionSource);
        Assert.DoesNotContain("IDeployFromTemplateWorkspaceControllerHost", fromTemplateCompositionSource);
        Assert.DoesNotContain("private readonly DeployFromTemplateWorkspaceController _controller;", fromTemplateCompositionSource);
        ...
        Assert.Contains("public async Task StartDeployAsync()", fromTemplateControllerSource);
        Assert.Contains("await _host.EnsureReferenceDataAsync(forceRefresh: false);", fromTemplateControllerSource);
        ...
        Assert.Contains("Task EnsureReferenceDataAsync(bool forceRefresh);", fromTemplateControllerHostSource);

        Assert.Contains("internal interface IDeployFromTemplateWorkspaceHost", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("Task EnsureTemplatesLibraryAsync(bool forceRefresh);", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("Task<TemplateEditorDocument> LoadTemplateForEditorAsync(string filePath);", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("Task<DeploymentReadinessReport> RunReadinessAsync(MultiVmDeploymentContext context, DeploymentPreflightMode mode);", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("void RefreshSharedUiState();", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceHost : IDeployFromTemplateWorkspaceHost", fromTemplateHostSource);
        Assert.DoesNotContain("DeployReferenceDataService", fromTemplateHostSource);
        Assert.DoesNotContain("DeployResolveSuggestionsService", fromTemplateHostSource);
        Assert.DoesNotContain("ApplyResolveSuggestionsAsync", fromTemplateHostSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceShellBridge", fromTemplateShellBridgeSource);
        Assert.Contains("public void RequestResultsPanelToggle() => _requestResultsPanelToggle();", fromTemplateShellBridgeSource);
        Assert.Contains("public void RefreshResultsPanelState() => _refreshResultsPanelState();", fromTemplateShellBridgeSource);

        Assert.Contains("public bool IsEvaluatingReadiness { get; private set; }", fromTemplateWorkspaceSource);
        Assert.Contains("public bool IsStarting { get; private set; }", fromTemplateWorkspaceSource);
        Assert.Contains("public string LifecycleState { get; private set; } = \"Idle\";", fromTemplateWorkspaceSource);
        Assert.Contains("public int ProgressPercent { get; private set; }", fromTemplateWorkspaceSource);
        Assert.Contains("public string ProgressSummary { get; private set; } = \"No deployment started.\";", fromTemplateWorkspaceSource);
        Assert.Contains("public void SetWorkflowState(", fromTemplateWorkspaceSource);

        Assert.Contains("internal static class DeployContextBuilder", deployContextBuilderSource);
        Assert.Contains("public static DeployContextBuildResult Build(", deployContextBuilderSource);
        Assert.Contains("private static DeployDiskResolution ResolveDeployDiskIdentity", deployContextBuilderSource);
        Assert.Contains("private static DeploySwitchResolution ResolveDeploySwitches", deployContextBuilderSource);

        Assert.Contains("internal sealed record DeployCompatibilityIssue(", deployContextTypesSource);
        Assert.Contains("internal sealed record DeployDiskResolution(", deployContextTypesSource);
        Assert.Contains("internal sealed record DeploySwitchResolution(", deployContextTypesSource);
        Assert.Contains("internal sealed record DeployContextBuildResult(", deployContextTypesSource);
    }
    ...
    public void DeployFromTemplate_UsesLocalProgressResultsStateSeam()
    {
        ...
        Assert.DoesNotContain("_deployFromTemplateWorkspaceComposition.RefreshResultRows(_deployCompatibilityIssues, _deployReadinessReport);", mainWindowSource);
    ...
    [Fact]
    public void DeployFromTemplate_UsesLocalCompositionHostCleanupSeam()
    {
        var mainWindowSource = LoadMainWindowSource();
        var deployWorkspaceCompositionSource = LoadDeployWorkspaceCompositionSource();
        var fromTemplateOwnerSource = LoadDeployFromTemplateWorkspaceOwnerSource();
        ...
        Assert.Contains("new DeployFromTemplateWorkspaceHost(", mainWindowSource);
        Assert.Contains("new DeployFromTemplateWorkspaceShellBridge(", mainWindowSource);
        Assert.DoesNotContain("UpdateDeployUi));", mainWindowSource);
        Assert.Contains("_deployResultsPanelCoordinator.ResetRightPanelBehavior();", mainWindowSource);
        ...
        Assert.DoesNotContain("_fromTemplateHost.Visibility = _shellBridge.IsDeployFromTemplateActive ? Visibility.Visible : Visibility.Collapsed;", deployWorkspaceCompositionSource);
        Assert.Contains("_fromTemplateWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployFromTemplateActive);", deployWorkspaceCompositionSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceOwner : IDeployFromTemplateWorkspaceControllerHost", fromTemplateOwnerSource);
        Assert.Contains("public void ApplyShellState(bool isFromTemplateActive)", fromTemplateOwnerSource);
        Assert.Contains("public void ResetPanelState()", fromTemplateOwnerSource);
        Assert.Contains("public void ApplyResultsPanelState(bool isActive, bool showPanel, bool panelUnavailable)", fromTemplateOwnerSource);
        Assert.Contains("public bool ShouldAutoOpenResultsPanel =>", fromTemplateOwnerSource);
        Assert.Contains("public string ResultsPanelTitle => \"From Template Progress / Results\";", fromTemplateOwnerSource);
        Assert.Contains("_shellBridge.RequestResultsPanelToggle();", fromTemplateOwnerSource);
        Assert.Contains("_shellBridge.RefreshResultsPanelState();", fromTemplateOwnerSource);

        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceComposition", fromTemplateCompositionSource);
        Assert.DoesNotContain("IDeployFromTemplateWorkspaceControllerHost", fromTemplateCompositionSource);
        Assert.Contains("public void SetVisibility(bool isActive)", fromTemplateCompositionSource);
        Assert.Contains("_view.Visibility = isActive ? Visibility.Visible : Visibility.Collapsed;", fromTemplateCompositionSource);
        ...
        Assert.Contains("public void ApplyResultsPanelState(bool isActive, bool showPanel, bool panelUnavailable)", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.OpenResultsPanelRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("void IDeployFromTemplateWorkspaceControllerHost.ApplyWorkspaceState()", fromTemplateCompositionSource);
        ...
        Assert.Contains("internal interface IDeployFromTemplateWorkspaceHost", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("bool IsTemplatesLoading { get; }", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("Task EnsureTemplatesLibraryAsync(bool forceRefresh);", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("Task<TemplateEditorDocument> LoadTemplateForEditorAsync(string filePath);", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("void RefreshSharedUiState();", fromTemplateWorkspaceHostInterfaceSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceHost : IDeployFromTemplateWorkspaceHost", fromTemplateHostSource);
        ...
        Assert.DoesNotContain("OnOpenResultsPanelRequested", fromTemplateHostSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceShellBridge", fromTemplateShellBridgeSource);
        Assert.Contains("public void RequestResultsPanelToggle() => _requestResultsPanelToggle();", fromTemplateShellBridgeSource);
        Assert.Contains("public void RefreshResultsPanelState() => _refreshResultsPanelState();", fromTemplateShellBridgeSource);
    ...
    public void DeployFromTemplate_ReducesBroadViewExposureAndUsesNarrowViewSeams()
    {
        ...
        Assert.DoesNotContain("WireDeployHandlers()", mainWindowSource);
        Assert.DoesNotContain("DeployFromTemplateView.EvaluateReadinessRequested += DeployEvaluateReadinessButton_Click;", mainWindowSource);
        Assert.DoesNotContain("DeployFromTemplateView.ResolveSuggestionsRequested += DeployResolveSuggestionsButton_Click;", mainWindowSource);
        Assert.DoesNotContain("DeployFromTemplateView.OpenTemplateEditorRequested += DeployOpenTemplateEditorButton_Click;", mainWindowSource);
        Assert.DoesNotContain("DeployFromTemplateView.StartDeployRequested += DeployStartButton_Click;", mainWindowSource);
        Assert.DoesNotContain("private async void DeployEvaluateReadinessButton_Click(object sender, RoutedEventArgs e)", mainWindowSource);
        Assert.DoesNotContain("private async void DeployResolveSuggestionsButton_Click(object sender, RoutedEventArgs e)", mainWindowSource);
        Assert.DoesNotContain("private async void DeployOpenTemplateEditorButton_Click(object sender, RoutedEventArgs e)", mainWindowSource);
        ...
        Assert.Contains("private void SetInteractionState(bool isLoadingTemplates, bool hasBlockingFailures)", fromTemplateCompositionSource);
        Assert.Contains("public void ApplyResultsPanelState(bool isActive, bool showPanel, bool panelUnavailable)", fromTemplateCompositionSource);
        Assert.Contains("_view.SetTemplateItemsSource(templateItemsSource);", fromTemplateCompositionSource);
        Assert.Contains("_view.SetSharedIssueSummariesItemsSource(_workspace.SharedIssueSummaries);", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.ReloadTemplatesRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.EvaluateReadinessRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.ResolveSuggestionsRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.OpenTemplateEditorRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.StartDeployRequested +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.TemplateSelectionChanged +=", fromTemplateCompositionSource);
        Assert.DoesNotContain("_view.OpenResultsPanelRequested +=", fromTemplateCompositionSource);
        ...
        Assert.Contains("private void WireHandlers()", fromTemplateOwnerSource);
        Assert.Contains("_view.ReloadTemplatesRequested += ReloadTemplatesRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.EvaluateReadinessRequested += EvaluateReadinessRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.ResolveSuggestionsRequested += ResolveSuggestionsRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.OpenTemplateEditorRequested += OpenTemplateEditorRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.StartDeployRequested += StartDeployRequested;", fromTemplateOwnerSource);
        Assert.Contains("_view.TemplateSelectionChanged += TemplateSelectionChanged;", fromTemplateOwnerSource);
    ...
    [Fact]
    public void DeployFromTemplate_ConvergedTestsProtectRefinedLocalArchitecture()
    {
        var mainWindowSource = LoadMainWindowSource();
        var deployWorkspaceCompositionSource = LoadDeployWorkspaceCompositionSource();
        var fromTemplateOwnerSource = LoadDeployFromTemplateWorkspaceOwnerSource();
        ...
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceViewModel", fromTemplateWorkspaceSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceController", fromTemplateControllerSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceOwner", fromTemplateOwnerSource);
        Assert.Contains("internal sealed class DeployFromTemplateWorkspaceComposition", fromTemplateCompositionSource);
        ...
        Assert.DoesNotContain("_fromTemplateHost.Visibility = _shellBridge.IsDeployFromTemplateActive ? Visibility.Visible : Visibility.Collapsed;", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("ApplyResolveSuggestionsAsync", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("TryToggleRightPanelFromWorkflow", deployWorkspaceCompositionSource);
    ...
    public void DeployOnTheFly_UsesLocalVmCollectionStateSeam()
    {
        ...
        Assert.DoesNotContain("private readonly DeployOnTheFlyWorkspaceViewModel _deployOnTheFlyWorkspace = new();", mainWindowSource);
        Assert.Contains("QuickDeployDraftCount: _deployOnTheFlyWorkspaceOwner.DraftCount,", mainWindowSource);
        Assert.Contains("_workspace.EnsureSeeded(_workspace.SelectedVmEntry?.VmId);", onTheFlyOwnerSource);
        Assert.Contains("_workspace.ReplaceEntriesFromTemplate(template, _workspace.SelectedVmEntry?.VmId);", onTheFlyOwnerSource);
        Assert.Contains("var entry = _workspace.AddVmEntry();", onTheFlyOwnerSource);
        Assert.Contains("_workspace.RemoveVmEntry(vmEntry);", onTheFlyOwnerSource);

        Assert.Contains("_onTheFlyWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployOnTheFlyActive);", deployWorkspaceCompositionSource);
    ...
    public void DeployOnTheFly_UsesLocalSelectionAndEditorDraftStateSeam()
    {
        ...
        Assert.DoesNotContain("_deployOnTheFlyWorkspace.SetSelectedVmEntry(", mainWindowSource);
        Assert.DoesNotContain("private void SelectDeployOnTheFlyVmEntry(VmTemplate? vmEntry)", mainWindowSource);
        Assert.DoesNotContain("private void UpdateDeployOnTheFlyEditorPanel()", mainWindowSource);
        Assert.DoesNotContain("private DeployOnTheFlyEditorViewState BuildDeployOnTheFlyEditorViewState()", mainWindowSource);
        ...
        Assert.DoesNotContain("_deployOnTheFlyWorkspaceController", mainWindowSource);
        Assert.Contains("_workspace.SelectedVmEntry is not null", onTheFlyCompositionSource);
        Assert.DoesNotContain("_deployOnTheFlyWorkspace.IsSynchronizingEditorDraft", mainWindowSource);
        Assert.Contains("_workspace.SetSelectedVmEntry(_view.CaptureVmSelectionInteractionState().SelectedVmEntryRow?.VmEntry);", onTheFlyOwnerSource);
        Assert.Contains("_composition.SyncEditorDraft(_view.CaptureEditorInteractionState());", onTheFlyOwnerSource);
        Assert.Contains("_controller.ScheduleAutoEvaluate();", onTheFlyOwnerSource);
        ...
        Assert.DoesNotContain("DeployOnTheFlyVmSwitchComboBox", mainWindowSource);
        Assert.DoesNotContain("DeployOnTheFlyVmVhdxCatalogComboBox", mainWindowSource);
        ...
        Assert.DoesNotContain("ApplyVmChangesRequested += ", onTheFlyCompositionSource);
        Assert.DoesNotContain("VmDraftChanged += ", onTheFlyCompositionSource);
        ...
        Assert.Contains("public readonly record struct DeployOnTheFlyVmSelectionInteractionState(", onTheFlyViewSource);
        Assert.Contains("public readonly record struct DeployOnTheFlyWorkspaceViewState(", onTheFlyViewSource);
        Assert.Contains("internal readonly record struct DeployOnTheFlyEditorInteractionState(", onTheFlyViewSource);
        Assert.Contains("internal readonly record struct DeployOnTheFlyEditorViewState(", onTheFlyViewSource);
        Assert.Contains("public event EventHandler? VmEntrySelectionChanged;", onTheFlyViewSource);
        Assert.Contains("public event EventHandler? VmDraftChanged;", onTheFlyViewSource);
        Assert.Contains("public DeployOnTheFlyVmSelectionInteractionState CaptureVmSelectionInteractionState()", onTheFlyViewSource);
        Assert.Contains("internal DeployOnTheFlyEditorInteractionState CaptureEditorInteractionState()", onTheFlyViewSource);
        Assert.Contains("public void SetVmEntrySelection(DeployOnTheFlyVmEntryRow? selectedVmEntryRow)", onTheFlyViewSource);
        Assert.Contains("internal void ApplyEditorViewState(DeployOnTheFlyEditorViewState state)", onTheFlyViewSource);
        Assert.Contains("public void ApplyWorkspaceState(DeployOnTheFlyWorkspaceViewState state)", onTheFlyViewSource);
        Assert.DoesNotContain("public TextBox DeployOnTheFlyVmNameTextBoxControl =>", onTheFlyViewSource);
        Assert.DoesNotContain("public ComboBox DeployOnTheFlyVmSwitchComboBoxControl =>", onTheFlyViewSource);
        Assert.DoesNotContain("public Button DeployOnTheFlyStartButtonControl =>", onTheFlyViewSource);
    }
    ...
    public void DeployOnTheFly_UsesLocalReadinessStateSeam()
    {
        ...
        Assert.Contains("_workspace.ReadinessReport is null", onTheFlyOwnerSource);
        Assert.Contains("!_workspace.IsEvaluatingReadiness", onTheFlyOwnerSource);
        ...
        Assert.Contains("_workspace.BeginReadinessEvaluation();", onTheFlyControllerSource);
        Assert.Contains("_workspace.ApplyReadinessResult(", onTheFlyControllerSource);
        Assert.Contains("_workspace.SetReadinessEvaluationFailed(\"Readiness evaluation failed.\");", onTheFlyControllerSource);
        Assert.Contains("await _host.EnsureReferenceDataAsync(forceRefresh: false);", onTheFlyControllerSource);
        Assert.Contains("var readinessReport = await _host.RunReadinessChecksAsync(deployContext.MultiVmContext, mode);", onTheFlyControllerSource);
        Assert.Contains("_host.UpdateUi();", onTheFlyControllerSource);
    ...
    public void DeployOnTheFly_UsesLocalDeployOrchestrationSeam()
    {
        ...
        Assert.DoesNotContain("private readonly DeployOnTheFlyWorkspaceController _deployOnTheFlyWorkspaceController;", mainWindowSource);
        Assert.DoesNotContain("_deployOnTheFlyWorkspaceController = new DeployOnTheFlyWorkspaceController(_deployOnTheFlyWorkspace, this);", mainWindowSource);
        ...
        Assert.Contains("internal sealed class DeployOnTheFlyWorkspaceOwner : IDeployOnTheFlyWorkspaceControllerHost", onTheFlyOwnerSource);
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceController _controller;", onTheFlyOwnerSource);
        Assert.Contains("_controller = new DeployOnTheFlyWorkspaceController(_workspace, this);", onTheFlyOwnerSource);
        Assert.Contains("private async void EvaluateRequested(object? sender, EventArgs e)", onTheFlyOwnerSource);
        Assert.Contains("private async void StartDeployRequested(object? sender, EventArgs e)", onTheFlyOwnerSource);
        Assert.Contains("await _controller.EvaluateReadinessAsync(DeploymentPreflightMode.Full);", onTheFlyOwnerSource);
        Assert.Contains("await _controller.StartDeployAsync();", onTheFlyOwnerSource);
        Assert.Contains("async Task<DeploymentOutcomeSummary> IDeployOnTheFlyWorkspaceControllerHost.DeployAllAsync(MultiVmDeploymentContext context)", onTheFlyOwnerSource);
        Assert.Contains("await _deploymentCoordinator.DeployAllAsync(context);", onTheFlyOwnerSource);
        Assert.Contains("return _deploymentOutcomeSummaryBuilder.Build(context);", onTheFlyOwnerSource);
    ...
    public void DeployOnTheFly_UsesLocalProgressAndResultsStateSeam()
    {
        ...
        Assert.Contains("_workspace.LifecycleState", onTheFlyCompositionSource);
        Assert.Contains("_workspace.ProgressPercent", onTheFlyCompositionSource);
        Assert.Contains("_workspace.ProgressSummary", onTheFlyCompositionSource);
        Assert.Contains("_workspace.IssueRows", onTheFlyCompositionSource);
        Assert.Contains("_workspace.LiveProgressVmCount", onTheFlyCompositionSource);
        ...
        Assert.Contains("_rightPanelView.SetResultRowsItemsSource(_workspace.ResultRows);", onTheFlyCompositionSource);
        ...
        Assert.Contains("_workspace.ResultRows.Count > 0", onTheFlyCompositionSource);
        Assert.Contains("_workspace.ResetProgressState();", onTheFlyOwnerSource);
        Assert.Contains("_workspace.SetShowAllVmRows(false);", onTheFlyOwnerSource);
        ...
        Assert.DoesNotContain("void IDeployOnTheFlyWorkspaceControllerHost.EnqueueUiUpdate(Action updateAction)", mainWindowSource);
        Assert.Contains("void IDeployOnTheFlyWorkspaceControllerHost.EnqueueUiUpdate(Action updateAction)", onTheFlyOwnerSource);
    ...
    [Fact]
    public void DeployOnTheFly_UsesLocalCompositionAndHostSeam()
    {
        var mainWindowSource = LoadMainWindowSource();
        var deployWorkspaceCompositionSource = LoadDeployWorkspaceCompositionSource();
        var onTheFlyCompositionSource = LoadDeployOnTheFlyWorkspaceCompositionSource();
        var onTheFlyOwnerSource = LoadDeployOnTheFlyWorkspaceOwnerSource();
        var onTheFlyShellBridgeSource = LoadDeployOnTheFlyWorkspaceShellBridgeSource();
        ...
        Assert.DoesNotContain("private void UpdateDeployOnTheFlyUi()", mainWindowSource);
        ...
        Assert.Contains("private void RequestDeployResultsPanelToggle()", mainWindowSource);

        Assert.Contains("private readonly DeployOnTheFlyWorkspaceOwner _onTheFlyWorkspaceOwner;", deployWorkspaceCompositionSource);
        Assert.Contains("DeployOnTheFlyWorkspaceOwner onTheFlyWorkspaceOwner,", deployWorkspaceCompositionSource);
        Assert.Contains("_onTheFlyWorkspaceOwner.SharedUiStateChanged += OnTheFlyWorkspaceOwner_SharedUiStateChanged;", deployWorkspaceCompositionSource);
        Assert.Contains("_onTheFlyWorkspaceOwner.ApplyShellState(_shellBridge.IsDeployOnTheFlyActive);", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("TryToggleRightPanelFromWorkflow", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("ShouldAutoOpenRightPanel", deployWorkspaceCompositionSource);
        Assert.DoesNotContain("ApplyRightPanelState(bool showPanel, bool panelUnavailable)", deployWorkspaceCompositionSource);
        ...
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceViewModel _workspace;", onTheFlyCompositionSource);
        Assert.Contains("_view.SetVmEntriesSource(_workspace.VmEntryRows);", onTheFlyCompositionSource);
        Assert.Contains("_rightPanelView.SetResultRowsItemsSource(_workspace.ResultRows);", onTheFlyCompositionSource);
        Assert.Contains("public void SetVisibility(bool isActive)", onTheFlyCompositionSource);
        Assert.Contains("public void ApplyResultsPanelState(bool isActive, bool showPanel, bool panelUnavailable)", onTheFlyCompositionSource);
        ...
        Assert.DoesNotContain("WireHandlers()", onTheFlyCompositionSource);
        Assert.Contains("internal sealed class DeployOnTheFlyWorkspaceOwner : IDeployOnTheFlyWorkspaceControllerHost", onTheFlyOwnerSource);
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceController _controller;", onTheFlyOwnerSource);
        Assert.Contains("private void WireHandlers()", onTheFlyOwnerSource);
        Assert.Contains("_view.ResolveSuggestionsRequested += ResolveSuggestionsRequested;", onTheFlyOwnerSource);
        Assert.Contains("_view.OpenTemplateEditorRequested += OpenTemplateEditorRequested;", onTheFlyOwnerSource);
        Assert.Contains("_view.StartDeployRequested += StartDeployRequested;", onTheFlyOwnerSource);
        Assert.Contains("_view.OpenResultsPanelRequested += OpenResultsPanelRequested;", onTheFlyOwnerSource);
        Assert.Contains("void IDeployOnTheFlyWorkspaceControllerHost.EnqueueUiUpdate(Action updateAction)", onTheFlyOwnerSource);
        Assert.Contains("internal sealed class DeployOnTheFlyWorkspaceShellBridge", onTheFlyShellBridgeSource);
        Assert.Contains("public void RequestResultsPanelToggle() => _requestResultsPanelToggle();", onTheFlyShellBridgeSource);
        Assert.Contains("internal sealed class DeployResultsPanelCoordinator", deployResultsPanelCoordinatorSource);
        Assert.Contains("private readonly DeployOnTheFlyWorkspaceOwner _onTheFlyWorkspaceOwner;", deployResultsPanelCoordinatorSource);
        Assert.Contains("_onTheFlyWorkspaceOwner.ApplyResultsPanelState(_isDeployOnTheFlyActive(), showPanel, panelUnavailable);", deployResultsPanelCoordinatorSource);
        Assert.False(File.Exists(GetDeployOnTheFlyWorkspaceHostPath()));
        Assert.False(File.Exists(GetDeployOnTheFlyCompositionHostPath()));
    ...
    public void DiagnosticsShellAndSharedComposition_PreserveLogsRouteAndDelegationBoundaries()
    {
        ...
        Assert.Contains("private string? TryOpenStructuredLogLocation(string filePath)", mainWindowSource);
        Assert.DoesNotContain("if (!IsDiagnosticsLogsActive || _isStructuredLogsLoading)", mainWindowSource);
        Assert.DoesNotContain("private readonly ObservableCollection<StructuredLogViewerEntry> _structuredLogEntries = [];", mainWindowSource);
        ...
        Assert.Contains("_logsWorkspaceComposition = new DiagnosticsLogsWorkspaceComposition(", compositionSource);
        Assert.Contains("_logsWorkspaceComposition.ApplyShellState();", compositionSource);
        Assert.Contains("_overviewWorkspaceComposition.ApplyShellState();", compositionSource);
        ...
        Assert.DoesNotContain("_overviewView.OpenSupportExportRequested", compositionSource);
        Assert.DoesNotContain("private readonly DiagnosticsLogsView _logsView;", compositionSource);
        Assert.DoesNotContain("private readonly DiagnosticsLogsWorkspaceViewModel _logsWorkspace = new();", compositionSource);
        Assert.DoesNotContain("private readonly DiagnosticsLogsWorkspaceController _logsController;", compositionSource);
        ...
        Assert.Contains("public string SupportSummaryText { get; }", overviewWorkspaceSource);
    ...
    private static string LoadDiagnosticsOverviewWorkspaceViewModelSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Diagnostics", "DiagnosticsOverviewWorkspaceViewModel.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }

    private static string LoadDiagnosticsLogsWorkspaceViewModelSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Diagnostics", "DiagnosticsLogsWorkspaceViewModel.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }
    private static string LoadDiagnosticsLogsWorkspaceControllerSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Diagnostics", "DiagnosticsLogsWorkspaceController.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }

    private static string LoadDiagnosticsLogsWorkspaceCompositionSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Diagnostics", "DiagnosticsLogsWorkspaceComposition.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }
    ...
    private static string LoadDiagnosticsLogsCodeBehindSource()
    {
    ...
    private static string GetDeployOnTheFlyWorkspaceHostPath()
    {
    ...
    private static string GetDeployOnTheFlyCompositionHostPath()
    {
    ...
    private static string LoadDeployContextBuilderSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Deploy", "DeployContextBuilder.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }
    ...
    private static string LoadDeployFromTemplateWorkspaceHostInterfaceSource()
    {
        var path = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "LabAssistant.WinUI", "ViewModels", "Deploy", "FromTemplate", "IDeployFromTemplateWorkspaceHost.cs");
        return File.ReadAllText(Path.GetFullPath(path));
    }
